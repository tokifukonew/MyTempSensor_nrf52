...
void DS1307_SetRegByte(uint8_t regAddr, uint8_t val) 
{
    ...
    ret_code_t err_code;
    err_code = nrf_drv_twi_tx(_ds1307_m_twi, DS1307_I2C_ADDR, DS1307_I2C_ADDR, 1, true);
    if (NRF_SUCCESS == err_code)
    {
        err_code = nrf_drv_twi_tx(_ds1307_m_twi, DS1307_I2C_ADDR, bytes, 2, true);
    }
}
...
uint8_t DS1307_GetRegByte(uint8_t regAddr) 
{
	uint8_t val;
    ret_code_t err_code;
    err_code = nrf_drv_twi_tx(_ds1307_m_twi, DS1307_I2C_ADDR, &regAddr, 1, true);
    if (NRF_SUCCESS == err_code)
    {
        err_code = nrf_drv_twi_tx(_ds1307_m_twi, DS1307_I2C_ADDR, &regAddr, 1, true);
    }
    if (NRF_SUCCESS == err_code)
    {
        err_code = nrf_drv_twi_rx(_ds1307_m_twi, DS1307_I2C_ADDR, &val, 1);
    }
	return val;
}
