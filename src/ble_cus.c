...
{ 
    ...
    if (p_evt_write->handle == p_our_service->char_handles_2.value_handle)
    {
        uint32_t *characteristic2_value_adr;
        int8_t characteristic2_value_val;
        characteristic2_value_adr = (uint32_t*) p_evt_write->data;
        characteristic2_value_val = *characteristic2_value_adr;

        NRF_LOG_INFO("characteristic2_value: %c", characteristic2_value_val);

        // Call the write handler function. Implementation is in the main.
        p_our_service->characteristic2_value_write_handler(characteristic2_value_val);
...
static uint32_t our_char_add(ble_os_t * p_our_service)
{
    // OUR_JOB: Step 2.A, Add a custom characteristic UUID
		...
		char_uuid.uuid      = BLE_UUID_CHARACTERISTIC_2;
...
void our_service_init(ble_os_t * p_our_service, ble_os_init_t * init)
{
    ...
    // OUR_JOB: Step 3.B, Set our service connection handle to default value. I.e. an invalid handle since we are not yet in a connection.
		p_our_service->conn_handle = BLE_CONN_HANDLE_INVALID;
p_our_service->characteristic2_value_write_handler = init->characteristic2_value_write_handler;
...
// ALREADY_DONE_FOR_YOU: Function to be called when updating characteristic value
void characteristic_value_update_2(ble_os_t *p_our_service, int32_t *temperature_value)
{
    // OUR_JOB: Step 3.E, Update characteristic value
		if (p_our_service->conn_handle != BLE_CONN_HANDLE_INVALID)
		{
