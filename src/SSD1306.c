/*
 * Based on afiskon/stm32-ssd1306
 */
#include "ssd1306.h"
#include "main.h"

nrf_drv_twi_t *_ssd1306_m_twi;

/**
 * @brief Initializes the SSD1306 module.
 * @param uint8_t write Byte
 */
void ssd1306_WriteCommand(uint8_t byte)
{
    ...
    buffer[0] = 0x00;
    buffer[1] = byte;
    err_code = nrf_drv_twi_tx(_ssd1306_m_twi, SSD1306_I2C_ADDR, buffer, sizeof(buffer), true);
    ...
    {
        err_code = nrf_drv_twi_tx(_ssd1306_m_twi, SSD1306_I2C_ADDR, &byte, 1, true);
    }
    nrf_delay_ms(1);
    // HAL_I2C_Mem_Write(_ssd1306_m_twi, SSD1306_I2C_ADDR, 0x00, 1, &byte, 1, true);
}

void ssd1306_WriteData(uint8_t *bufp, size_t buff_size)
{
    ret_code_t err_code;
    uint8_t buffer[1 + buff_size];
    memset(buffer, 0x40, 1);
    memcpy(buffer + 1, bufp, buff_size);
